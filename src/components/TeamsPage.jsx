...
function CreateTeamModal({ isOpen, onClose, allUsers, onTeamCreated }) {
...
function ChangeMemberTeamModal({ isOpen, onClose, member, allTeams, onMoved }) {
  ...
  const otherTeams = allTeams.filter(t => !t.memberIds.includes(member.id));
  ...
  async function handleMove(teamId) {
    ...
    try {
      ...
      onMoved(); // Trigger re-load of everything
      onClose();
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <CreateTeamModal 
        ...
        allUsers={allUsers}
        onTeamCreated={() => loadData()} // Simple reload all for clean sync
      />
